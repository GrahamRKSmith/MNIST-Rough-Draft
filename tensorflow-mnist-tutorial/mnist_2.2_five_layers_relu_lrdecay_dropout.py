...
#          ·                                                        Y5 [batch, 10]
#GS: Much of the code is the same, until we get to dropout in the middle
...
#GS: the setup is the same. We create the placeholders for our data, but this time we also define a new variable called pkeep
#GS: This value represents the probability of keeping any particular neuron during dropout.
...
#GS: here the differences start with dropout
Y1 = tf.nn.relu(tf.matmul(XX, W1) + B1)
#GS: When we do dropout, we are basically randomly killing off a random percentage of neurons in each layer
#GS: In this case, the value for how many neurons we wish to keep is pkeep. A common value is 0.5.
#GS: It is another one of those hyper parameters you have to experiment with to find out which values work best.
#GS: The reason dropout is used is to prevent overfitting - it is essentially a form of regularization
#GS: For example, if we have a model that performs very well on our training set, but performs poorly on the test set,
#GS: we should be very suspicious - it is likely that the model overfit the training data and does not generalize well to new data.
...
#GS: This is the function definition for the training step, which means that everything inside this function is going to be performed with each iteration
#GS: In this program there are 10001 iterations, so i takes on the values from 0 to 10000
#GS: The number of iterations is something that you decide. Sometimes a network converges quickly, but sometimes it can take hours or days.
#GS: You will generally need a lot of iterations.
#GS: The update_test_data parameter is a boolean value which indicates whether you want to run testing.
#GS: and likewise with update_train_data.
